...
# Fix macOS Gatekeeper quarantine on the (unsigned) LexiKeep app bundle.
#
...
APP_PATH="${1:-/Applications/LexiKeep.app}"

if [ ! -d "$APP_PATH" ]; then
  echo "错误：未找到应用 $APP_PATH" >&2
  echo "请确认已将 LexiKeep.app 拖入「应用程序」文件夹，或指定正确路径：" >&2
  echo "  bash $0 /path/to/LexiKeep.app" >&2
  exit 1
fi
