...
LexiKeep 是一款免费、开源、本地优先的阅读学习工具。导入一篇文章、一本书或影视字幕——读到不懂的单词随手点一下就有释义，再用智能复习把它们真正记下来。
...
以上只要有一条符合，LexiKeep 就是为你做的。无需账号，核心学习流程离线可用。
...
4. **复习** — LexiKeep 在合适的时候轻轻提醒你，每天几分钟
...
- **AI 可选** — 支持本地 Ollama、Qwen 及其他 OpenAI 兼容 API，用于段落解释和翻译，默认关闭
...
LexiKeep 免费且开源。如果它对你有所帮助，欢迎在 GitHub 上点 Star 或参与贡献：[github.com/Pnut-You/LexiKeep](https://github.com/Pnut-You/LexiKeep)

## 开发

面向贡献者。需要 Node.js 20+ 与 Rust 1.77.2+。

React 前端位于 `src/`，Tauri/Rust 后端位于 `src-tauri/src/`，两个目录的分工是有意设计。产品范围与决策见 [docs/PRODUCT.md](docs/PRODUCT.md)。
