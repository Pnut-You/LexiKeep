...
LexiKeep 的预编译安装包由 [GitHub Actions](.github/workflows/build-installers.yml) 手动触发构建。由于目前尚未购买 Apple Developer / Windows 代码签名证书 / Google Play 开发者账号，安装包暂时为**未签名版本**，各平台系统会给出安全拦截提示，属预期行为。
...
- 「“LexiKeep”已损坏，无法打开。你应该将它移到废纸篓。」
- 「无法打开“LexiKeep”，因为 Apple 无法检查其是否包含恶意软件。」
...
1. 双击 DMG，将 **LexiKeep.app** 拖入「应用程序」（Applications）文件夹。
2. 用任一方式移除隔离标记（**任选其一即可**，每个副本只需一次）：
   ...
   **方式 A：一键脚本（推荐）**
   ...
   > 脚本默认修复 `/Applications/LexiKeep.app`；若 App 在其他位置，可传入路径：
   > `bash scripts/fix-macos-quarantine.sh /path/to/LexiKeep.app`

   **方式 B：命令行**

   ```bash
   xattr -dr com.apple.quarantine "/Applications/LexiKeep.app"
   ```

   **方式 C：右键打开**

   在「应用程序」中按住 **Control** 并点击 **LexiKeep**，选择「打开」，然后在弹窗中点击「打开」。
...
1. 双击安装包 `LexiKeep_*_x64-setup.exe`。
...
```bash
sudo apt install ./LexiKeep_1.1.3_amd64.deb
```
